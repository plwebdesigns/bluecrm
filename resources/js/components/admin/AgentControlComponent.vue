<template>
  <div class="container increased-width">
            ...
            <tr v-for="(agent, index) in formattedAgents" :key="index">
              <td>
                <input
                  ...
                  name="exampleRadios"
                  :id="'row-' + index"
                  v-on:input="radioSelected($event)"
                />
    ...
    <div class="row">
      <div class="col-9">
        <h1 class="font-fredricka text-center">Inactive Agents</h1>
        <div v-if="message !== ''" class="col-3 alert alert-success">
          {{ message }}
          <button
            class="btn btn-sm btn-outline-dark ml-1"
        ...
        <table class="table table-sm table-borderless">
          ...
          <tbody>
            <tr v-for="(inactive_agent, index) in inactive_agents" :key="index">
              <td>
                <input
                  class="form-check-input"
                  type="radio"
                  ...
                  v-on:input="radioSelected($event)"
                />
                <label class="form-check p-0">{{ inactive_agent.id }}</label>
              </td>
              <td>
                <input
                  :id="'agent_name-' + index"
                  class="form-control form-control-sm"
                  type="text"
                  v-model="inactive_agent.agent_name"
                  readonly
                />
              </td>
              <td>
                <select
                  class="custom-select custom-select-sm"
                  v-model="inactive_agent.title"
                  :id="'title-' + index"
                  disabled
                >
                  <option selected>{{ inactive_agent.title }}</option>
                  <option v-for="(item, index) in titles" :key="index">
                    {{ item.title }}
                  </option>
                </select>
              ...
              <td>
                <div class="input-group">
                  <input
                    ...
                    type="text"
                    style="max-width: 60px"
                    v-model="inactive_agent.current_split"
                    readonly
                  />
                  <div class="input-group-append" style="max-height: 25px">
                    <span class="input-group-text">%</span>
              ...
              <td>
                <div class="input-group">
                  ...
                  <input
                    ...
                    type="text"
                    style="max-width: 80px"
                    v-model="inactive_agent.membership_fee"
                    readonly
                  />
                </div>
              </td>
              ...
              <td class="pr-0">
                ...
                <button
                  ...
                  :id="'savebtn-' + index"
                  class="btn btn-sm btn-primary"
                  v-on:click="save(inactive_agent, index)"
                  hidden
                >
                  Save
                </button>
  ...
  methods: {
    ...
    getAgents: function () {
      ...
      $.ajax({
        ...
        this.inactive_agents = resp.inactive_agents;
        this.number_pages = this.agents.length;
      });
    },
